...
async function run() {
  ...
  const rows = await db.all(\`
    SELECT categoryId, COUNT(*) as cnt, SUM(amount) as total, currency
    FROM transactions WHERE categoryId LIKE 'custom:%'
    GROUP BY categoryId, currency ORDER BY cnt DESC
  \`);
  function parseName(id) { try { return decodeURIComponent(id.slice(7).split('|')[0]); } catch { return id; } }
  for (const r of rows) console.log(parseName(r.categoryId) + ': ' + r.cnt + ' tx, ' + Math.round(r.total) + ' ' + r.currency);
  await db.close();
}
