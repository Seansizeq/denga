...
async function run() {
  ...
  const enc = encodeURIComponent('Цифрові');
  const { changes } = await db.run(
    "UPDATE transactions SET categoryId = 'other_expense' WHERE categoryId LIKE 'custom:%' AND categoryId LIKE '%' || ? || '%'",
    [enc]
  );
  await db.run("DELETE FROM custom_categories WHERE normalized_name = 'цифрові'");
  console.log('Перенесено:', changes, 'tx -> Інше');
  await db.close();
}
