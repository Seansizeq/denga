...
const SUBSCRIPTIONS_ID = 'custom:' + encodeURIComponent('Підписки') + '|Wifi|' + encodeURIComponent('#5E5CE6');
...
async function run() {
  ...
  const { changes } = await db.run(
    "UPDATE transactions SET categoryId = ? WHERE categoryId LIKE 'custom:%' AND categoryId LIKE '%' || ? || '%'",
    ['${SUBSCRIPTIONS_ID}', encodeURIComponent('Мобільний')]
  );
  console.log('Перенесено транзакцій:', changes);
  await db.close();
}
