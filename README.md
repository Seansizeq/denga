...
Приложение работает двумя процессами pm2 (`ecosystem.config.cjs`):

| Процесс | Что делает | Копий |
| --- | --- | --- |
| `denga-api` | только HTTP | несколько (`cluster`) |
| `denga-bot` | опрос Telegram, планировщики, отчёты, бэкапы | **ровно одна** (`fork`) |

Разделение не косметическое. Отрисовка карточки отчёта — это около полусекунды
сплошного счёта, и пока она шла в том же процессе, что отвечает приложению, он
не отвечал никому. А поднять вторую копию было нельзя: вместе с ней поднялся бы
второй опрос Telegram, а два поллера на один токен делят апдейты между собой —
половина сообщений теряется. Поэтому `denga-bot` строго один.

API в Telegram напрямую не пишет: сообщения кладутся в таблицу
`telegram_outbox`, откуда их забирает бот-процесс. Побочная выгода — сообщения
переживают перезапуск.

```bash
pm2 start ecosystem.config.cjs     # первый запуск
pm2 reload denga-api               # обновление API без простоя
pm2 restart denga-bot
```

`npm run deploy` делает это сам и умеет перейти со старой однопроцессной
раскладки: если найдёт процесс из `DEPLOY_PM2_NAME`, удалит его и поднимет
ecosystem.

Локально всё по-прежнему можно запустить одним процессом — `npm run server`
(роль `all`). Отдельные роли: `npm run api`, `npm run bot`.
