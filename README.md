...
У чаті з ботом можна додавати витрати вільним текстом або скріншотом
банківської операції. Для скріншота потрібні `OCR_SPACE_API_KEY` і налаштований
`SMART_TRANSACTION_PROVIDER`; перед збереженням бот завжди показує картку
підтвердження з кнопками «Зберегти», «Змінити» та «Скасувати».
