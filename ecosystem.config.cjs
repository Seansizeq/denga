 ...
 * Скільки копій API.
 *
 * `max` віддав би всі ядра, не лишивши нічого бот-процесу — а саме він
 * зайнятий важким рахунком. Тому рахуємо від кількості ядер із запасом на
 * бота: на двоядерному VPS це одна копія, на чотириядерному — дві.
 *
 * Стеля у дві копії свідома. Записи SQLite однаково серіалізує на рівні файлу
 * (виміряно: 277 req/s на одному процесі проти 263 на двох), тож більше копій
 * прискорює лише читання — а їх після переходу на ETag і так майже немає.
 */
const os = require('node:os');
const API_INSTANCES =
  Number(process.env.DENGA_API_INSTANCES) || Math.max(1, Math.min(2, os.cpus().length - 1));
