...
const remoteDeploy = async (expectedSha) => {
  ...
  const script = [
    ...
    '  echo "[server] Refusing deploy: $APP_DIR is not an existing git checkout." >&2',
    '  exit 1',
    'fi',
    'cd "$APP_DIR"',
    'if ! git diff --quiet || ! git diff --cached --quiet; then',
    '  echo "[server] Refusing deploy: tracked server files have local changes." >&2',
    '  git status --short --untracked-files=no >&2',
    '  exit 1',
    'fi',
    'echo "[server] Fetching latest..."',
