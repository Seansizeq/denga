...
const normalizeDebtDirection = (value) => {
  const v = String(value ?? '').trim();
  if (v === 'owed_to_me' || v === 'i_owe') return v;
  return '';
};
...
const dispatchReminder = async (dbConn, userId, reminder, timeZone, chatId) => {
  if (!bot) return;
  ...
  if (kind === 'shift_evening_before') {
    const offset = Math.max(1, Math.min(14, Number(reminder.leadDays) || 1));
    const targetDay = shiftIsoDay(today, offset);
    const hit = await dbConn.get(
      `SELECT 1 AS ok FROM planner_days WHERE user_id = ? AND day = ? AND has_shift = 1 LIMIT 1`,
      [userId, targetDay]
    );
    if (!hit?.ok) return;
    await bot.sendMessage(
  ...
  if (kind === 'shift_unclosed') {
    ...
    const started = new Date(String(active.startedAt));
    if (Number.isNaN(started.getTime())) return;
    const hoursOpen = (Date.now() - started.getTime()) / (3600 * 1000);
  ...
  for (const u of users) {
    ...
    for (const reminder of reminders) {
      ...
      if (await markReminderDelivery(db, userId, reminder.id, slot)) {
        await dispatchReminder(db, userId, reminder, tz, chatId);
      }
    }
  }
}
...
app.get('/api/debts', async (req, res) => {
  ...
  const status = normalizeDebtStatus(req.query?.status) || 'open';
...
app.post('/api/debts', async (req, res) => {
  const userId = req.authUserId;
  const direction = normalizeDebtDirection(req.body?.direction);
  const personName = typeof req.body?.personName === 'string' ? req.body.personName.trim().replace(/\s+/g, ' ') : '';
  const amount = parseAmount(req.body?.amount);
  const currency = normalizeCurrency(req.body?.currency);
  const dueDateRaw = typeof req.body?.dueDate === 'string' ? req.body.dueDate.trim() : '';
  const dueDate = dueDateRaw ? toIsoDate(parseIsoDate(dueDateRaw) ?? new Date('invalid')) : null;
  const note = typeof req.body?.note === 'string' ? req.body.note.trim() : '';

  if (!direction) {
    res.status(400).json({ error: 'direction must be owed_to_me or i_owe' });
    return;
  }
  if (!personName) {
  ...
  if (note.length > 140) {
    res.status(400).json({ error: 'note must be <= 140 chars' });
    return;
  }
  ...
  await db.run(
    `INSERT INTO debt_records
     (id, user_id, direction, person_name, amount, currency, due_date, note, status, closed_at, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'open', NULL, ?, ?)`,
    [id, userId, direction, personName, amount, currency, dueDate, note || null, now, now]
  );
  const row = await db.get(
    `SELECT
       id,
...
app.patch('/api/debts/:id', async (req, res) => {
  ...
  const existing = await db.get(
    `SELECT id, status FROM debt_records WHERE user_id = ? AND id = ? LIMIT 1`,
    [userId, id]
  );
  if (!existing) {
  ...
  if (req.body?.currency !== undefined) {
    ...
    params.push(normalizeCurrency(req.body.currency));
  }
  ...
  if (req.body?.dueDate !== undefined) {
    ...
    if (!dueDateRaw) {
      ...
      const parsed = parseIsoDate(dueDateRaw);
      if (!parsed) {
        res.status(400).json({ error: 'dueDate must be YYYY-MM-DD' });
        return;
      }
      updates.push('due_date = ?');
      params.push(toIsoDate(parsed));
    }
  }
  ...
  if (req.body?.status !== undefined) {
    const status = normalizeDebtStatus(req.body.status);
    ...
    params.push(status === 'closed' ? new Date().toISOString() : null);
  }
...
app.delete('/api/debts/:id', async (req, res) => {
  ...
  const existing = await db.get('SELECT id FROM debt_records WHERE user_id = ? AND id = ? LIMIT 1', [userId, id]);
  if (!existing) {
    res.status(404).json({ error: 'Debt not found' });
    return;
  }
  await db.run('DELETE FROM debt_records WHERE user_id = ? AND id = ?', [userId, id]);
  res.status(204).end();
});
