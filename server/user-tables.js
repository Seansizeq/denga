...
export const USER_TABLES = [
  // Перед `transactions`: рядок у `bank_inbox` посилається на транзакцію, яку
  // з неї записали.
  'bank_inbox',
  'bank_merchant_rules',
  // Разом із цим рядком зникає й збережений токен банку — іншого місця, де він
  // лежить, немає. Сам вебхук у банку при цьому лишається ввімкненим: зняти
  // його можна лише запитом до monobank, а мережевий виклик усередині
  // видалення акаунта зробив би неподільну операцію залежною від чужого
  // сервера. Операції з нього вже нікуди не потраплять — маршрут не знайде
  // привʼязки й відповість 404.
  'bank_links',
  'transactions',
  'custom_categories',
  'category_prefs',
