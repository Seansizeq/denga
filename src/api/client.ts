...
const bybitResponse = async (response: Response): Promise<BybitCardStatus> => {
...
export const getBybitCardStatus = async (): Promise<BybitCardStatus> =>
  bybitResponse(await apiFetch('/api/integrations/bybit-card'));

export const connectBybitCard = async (
  apiKey: string,
  apiSecret: string,
): Promise<BybitCardStatus> =>
  bybitResponse(
    await apiFetch('/api/integrations/bybit-card', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export const syncBybitCard = async (): Promise<BybitCardStatus> =>
  bybitResponse(
    await apiFetch('/api/integrations/bybit-card/sync', {
      method: 'POST',
    }),
  );

export const disconnectBybitCard = async (): Promise<void> => {
  const response = await apiFetch('/api/integrations/bybit-card', { method: 'DELETE' });
  if (!response.ok) {
    let message = 'Could not disconnect Bybit Card';
    try {
      const body = (await response.json()) as { error?: string };
      if (body.error) message = body.error;
    } catch {
