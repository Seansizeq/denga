...
const dictionary: Dict = {
  ...
  addTx: {
    ...
    chargedFromAccount: 'Со счёта спишется',
    creditedToAccount: 'На счёт зачислится',
  ...
  planner: {
    ...
    salaryRate: 'Ставка за час',
    salaryAmount: 'Сумма за смену',
    note: 'Заметка',
    ...
    shiftModeHours: 'Часами',
    shiftDurationInvalid: 'Укажите разные начало и конец смены',
    shiftOvernightHint: ' · конец раньше начала означает ночную смену',
    templateNew: 'Новый шаблон',
    templateAdd: 'Новый',
    templateEdit: 'Шаблон смены',
    templatesEmpty: 'Шаблонов пока нет. Создайте первый — и смену можно будет добавить одним касанием.',
    templatesHint: 'Шаблон подставляет время, ставку и название в новую смену. Меняйте его здесь — уже записанные из него смены остаются как есть.',
    templateDefaultTag: 'По умолчанию',
    templateExists: 'Такой шаблон уже есть — измените название, символ или валюту.',
    manageTemplates: 'Шаблоны смен',
    startShiftNow: 'Начнётся сейчас, независимо от выбранного дня',
    saveAsTemplate: 'Сохранить как шаблон',
  },
  subscriptions: {
    title: 'Подписки',
  ...
  settings: {
    ...
    currencyDescription:
      'В ней показываются суммы. В ней же считаются записи из бота и ярлыка, когда валюта не указана.',
    currencyUah: 'Гривна (₴)',
    currencyPln: 'Злотый (zł)',
    currencyUsd: 'Доллар ($)',
    display: 'Отображение',
    ...
    automationExpenseDescription:
      'Ярлык на телефоне берёт по ссылкам ваши категории, счета и валюты, а по последней сохраняет расход. Списки подтягиваются каждый раз, так что новую категорию или счёт не нужно вписывать в ярлык руками.',
    automationCategoriesUrl: 'Ссылка: список категорий',
    automationAccountsUrl: 'Ссылка: список счетов',
    automationCurrenciesUrl: 'Ссылка: список валют',
    automationTransactionUrl: 'Ссылка: добавить расход',
    automationExpenseHowTo:
      'iOS, на каждый список: «Получить содержимое URL» со ссылкой → сразу «Выбрать из списка». Затем «Получить содержимое URL» с последней ссылкой, метод POST, тело JSON: amount (число из «Запросить ввод»), categoryId и account — просто выбранные строки из списков.',
    automationExpenseHowToCurrency:
      'Поле currency необязательное: без него сумма считается в валюте по умолчанию из настроек. Добавьте список валют в ярлык — и можно платить злотыми с гривневой карты: спишется по курсу дня.',
    ...
    automationWalletTitle: 'Оплата картой через Apple Pay',
    automationWalletDescription:
      'Автоматизация срабатывает сама, как только вы прикладываете телефон к терминалу: сумму и название магазина передаёт Wallet, категорию приложение угадывает, а бот присылает карточку — исправить одним касанием. Исправленную категорию он запоминает для этого магазина.',
    automationWalletHowTo:
      'iOS: Команды → Автоматизация → Новая → «Кошелёк» (в старых версиях «Транзакция») → выберите карту → «Запускать сразу». Действия: «Найти совпадение в тексте» по Transaction Amount с выражением [0-9.,]+ → «Получить содержимое URL» со ссылкой, метод POST, тело JSON.',
    automationWalletHowToCard:
      'Поля JSON: amount — найденное число, currency — валюта карты (UAH, PLN или USD), merchant — Transaction Name, account — ключ вашего счёта. Категорию не передавайте: именно тогда приходит карточка с исправлением.',
    automationWalletLimits:
      'Ловит только касания картой в магазине. Оплаты в приложениях и в браузере Apple не передаёт — их добавьте вручную или привяжите карту банка выше.',
    bankTitle: 'Карта банка',
    bankDescription:
      'Привязанная карта записывает операции сама — и покупки в магазине, и онлайн, и подписки. Бот присылает карточку с угаданной категорией, чтобы исправить её одним касанием.',
    bankWritesTo: 'Записывает в',
    bankTokenLabel: 'Персональный токен monobank',
    bankTokenPlaceholder: 'u_XXXXXXXXXXXXXXXXXXXXXX',
    bankTokenHint:
      'Откройте api.monobank.ua с телефона, войдите через приложение monobank и скопируйте токен. Он даёт доступ только на чтение выписки.',
    bankLoadAccounts: 'Показать карты',
    bankChooseAccount: 'Какую карту подключить',
    bankChooseWallet: 'Куда записывать операции',
    bankConnect: 'Подключить',
    bankConnected: 'Карта подключена',
    bankDisconnect: 'Отключить',
    bankUnsupportedCurrency: 'Валюта не поддерживается',
    bankNoWalletAccount:
      'Нет счёта в этой валюте. Создайте его на экране «Счета» — именно туда пойдут операции.',
    bankHowTo:
      'Если на эту же карту настроена и автоматизация Apple Pay, отключите одно из двух: иначе каждая покупка запишется дважды.',
