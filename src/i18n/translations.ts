...
const uk: Dict = {
  ...
  settings: {
    ...
    bybitErrorCardRequest: 'Bybit не надає історію Card для цього акаунта через API. Баланси синхронізуються, але покупки поки не імпортуються. Перевірте, що картка активна, і спробуйте пізніше.',
...
const ru: Dict = {
  ...
  settings: {
    ...
    bybitErrorCardRequest: 'Bybit не предоставляет историю Card для этого аккаунта через API. Балансы синхронизируются, но покупки пока не импортируются. Проверьте, что карта активна, и повторите позже.',
...
const en: Dict = {
  ...
  settings: {
    ...
    bybitErrorCardRequest: 'Bybit does not provide Card history for this account through the API. Balances still sync, but purchases are not imported yet. Check that the card is active and retry later.',
